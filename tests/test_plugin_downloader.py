...
def test_download_and_extract_succeeds_with_relative_redirect(monkeypatch, tmp_path):
    ...
    session = _FakeSession(
        [
            _FakeResponse(is_redirect=True, location="/plugin.zip", status_code=302),
            _FakeResponse(chunks=[archive_bytes], status_code=200),
        ]
    )

    monkeypatch.setattr("downloaders.plugin_downloader.get_session", lambda: session)
    monkeypatch.setattr(PluginDownloader, "_validate_url", lambda self, url: None)
    ...
    assert session.calls[0]["url"].startswith("https://downloads.wordpress.org/plugin/")
    assert session.calls[1]["url"] == "https://downloads.wordpress.org/plugin.zip"
...
def test_download_and_extract_returns_none_on_zip_slip(monkeypatch, tmp_path):
    ...
    session = _FakeSession([_FakeResponse(chunks=[malicious_payload.getvalue()], status_code=200)])
