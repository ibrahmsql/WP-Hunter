...
from models import ScanConfig, ScanStatus, PluginResult
...
def _request_scanner_stop(scanner: Any) -> None:
...
async def run_scan_task(session_id: int, config: ScanConfig, repo: ScanRepository):
    ...
    try:
        ...
        scanner = active_scans.get(session_id)
        session_state = repo.get_session(session_id)
        already_cancelled = bool(
            session_state and session_state.get("status") == ScanStatus.CANCELLED.value
        )
        cancel_requested = session_id in cancel_requests
        if already_cancelled or cancel_requested:
...
async def delete_scan(session_id: int):
    ...
    # Stop if running
    if session_id in active_scans:
        _request_scanner_stop(active_scans[session_id])
        del active_scans[session_id]
