...
class UpdateManager:
    ...
    def _deploy_release(self, release_root: Path) -> None:
        if not (release_root / "server").exists():
            raise RuntimeError(
                "Release archive does not contain expected project files."
            )

        copied_entries = 0
        for child in release_root.iterdir():
            if (
                child.name in self.DEPLOY_EXCLUDE_DIRS
                or child.name in self.DEPLOY_EXCLUDE_FILES
            ):
                continue
            ...
            if child.is_dir():
                shutil.copytree(
                    ...
                    ignore=shutil.ignore_patterns(
                        *self.DEPLOY_EXCLUDE_DIRS, *self.DEPLOY_EXCLUDE_FILES
                    ),
                )
                copied_entries += 1
            else:
        ...
        self._last_update_message = (
            "Release files copied. Please restart the dashboard."
        )
    ...
    def get_status(self, force: bool = False) -> Dict:
        ...
        already_installed = bool(
            installed_tag
            and latest_tag
            and self._normalized_version(installed_tag)
            == self._normalized_version(latest_tag)
        )
        update_available = (
            bool(release.get("update_available")) and not already_installed
        )
        status = {
            ...
            "checked_at": (self._cache_time.isoformat() + "Z")
            if self._cache_time
            else None,
    ...
    def start_update(self) -> str:
        ...
        try:
            ...
            download_url = release_status.get("download_url") or release_status.get(
                "asset_url"
            )
            if not download_url:
    ...
    def _run_update(self, asset_url: str, latest_tag: str) -> None:
        ...
        try:
            ...
            self._last_update_message = (
                "Update complete. Restart the dashboard to pick up the new version."
            )
            self._last_error = None
        except Exception as exc:
            self._last_error = f"{type(exc).__name__}: {exc}"
