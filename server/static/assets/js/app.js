...
function connectWebSocket(sessionId) {
    ...
    ws.onmessage = (event) => {
        ...
        handleMessage(payload, sessionId);
    };
    ws.onclose = () => {
        if (socket !== ws) return;
        logTerminal('WebSocket connection closed', 'info');
        reconcileScanStatus(sessionId);
    };
    ws.onerror = () => {
        if (socket !== ws) return;
...
async function reconcileScanStatus(sessionId) {
    if (!sessionId) return;
    if (currentScanId !== sessionId) return;
    ...
    try {
        ...
        if (status === 'completed') {
            ...
            setScanProgressState(100, 'Completed', `Found ${totalFound} total / ${highRiskCount} high risk`);
            setProgressDetailsButton(true);
            refreshAfterScanEvent(sessionId);
        } else if (status === 'cancelled') {
            document.getElementById('scan-status').textContent = 'CANCELLED';
            document.getElementById('scan-status').className = 'metric-value info-value cancelled';
            setScanProgressState(100, 'Cancelled', `Found ${totalFound} total / ${highRiskCount} high risk`);
            setProgressDetailsButton(true);
            refreshAfterScanEvent(sessionId);
            ...
            setScanProgressState(100, 'Failed', 'Scan terminated with an error');
            setProgressDetailsButton(false);
            refreshAfterScanEvent(sessionId);
        }
    } catch (error) {
...
function handleMessage(msg, sourceSessionId = null) {
    const runBtn = document.getElementById('runBtn');
    if (sourceSessionId && currentScanId && Number(sourceSessionId) !== Number(currentScanId)) {
        return;
    }
    if (msg && msg.session_id && currentScanId && Number(msg.session_id) !== Number(currentScanId)) {
        return;
    }
    ...
    switch(msg.type) {
        ...
        case 'deduplicated':
            ...
            break;
        case 'cancelled':
            logTerminal(`Scan cancelled. Found: ${msg.total_found || 0}, High Risk: ${msg.high_risk_count || 0}`, 'warn');
            document.getElementById('scan-status').textContent = 'CANCELLED';
            document.getElementById('scan-status').className = 'metric-value info-value cancelled';
            runBtn.disabled = false;
            runBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg><span>RUN SCAN</span>';
            setScanProgressState(100, 'Cancelled', `Found ${msg.total_found || 0} total / ${msg.high_risk_count || 0} high risk`);
            setProgressDetailsButton(true);
            refreshAfterScanEvent(currentScanId);
            if (detailsPollingInterval) {
                clearInterval(detailsPollingInterval);
...
window.openPluginModal = function(index, options = {}) {
    ...
    const pluginSlugJs = JSON.stringify(String(plugin.slug || ''));
    const downloadUrlJs = JSON.stringify(String(downloadUrl || ''));
    ...
    content.innerHTML = `
        ...
        <section class="plugin-toolbar">
            ...
            <div class="plugin-toolbar-actions">
                <button id="plugin-fav-btn" onclick='toggleFavorite(${pluginSlugJs})' class="action-btn details-fav-btn plugin-fav-inline${isFav ? ' active' : ''}" title="${isFav ? 'In Favorites' : 'Add to Favorites'}" aria-label="${isFav ? 'Remove from' : 'Add to'} favorites">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                </button>
                <a href="${escapeHtml(downloadUrl)}" target="_blank" rel="noreferrer noopener" class="action-btn plugin-action-download">
                    <span>DOWNLOAD ZIP</span>
                </a>
                <button id="btn-deep-scan" onclick='runPluginSemgrep(${pluginSlugJs}, ${downloadUrlJs})' class="action-btn plugin-action-secondary">
